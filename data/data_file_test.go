...
func TestOpenDataFile(t *testing.T) {
	...
	dataFile1, err := OpenDataFile(os.TempDir(), 11, fio.StandardIO)
	assert.Nil(t, err)
	assert.NotNil(t, dataFile1)

	dataFile2, err := OpenDataFile(os.TempDir(), 11, fio.StandardIO)
	assert.Nil(t, err)
...
func TestDataFile_Sync(t *testing.T) {
	...
	dataFile, err := OpenDataFile(os.TempDir(), 12, fio.StandardIO)
	assert.Nil(t, err)
	assert.NotNil(t, dataFile)
